...
RAND_COMMIT = "C:/Data/092019 CommitInfo/RANDJava_Commit.xlsx"
...
def clean_files(x):
    ...
    if pd.notna(x['PINDEX']): return x
    
    if pd.notna(x['OPEN_ISSUES']):
...
def main():
    ...
    for xl_sheet in REPOCOMMIT_LIST:
        ...
        r_writer = pd.ExcelWriter(RAND_COMMIT , engine='xlsxwriter')
        ...
        for start in i_list:
            ...
            for i, row in df_commit.loc[start:].iterrows():
                ...
                r_commits = r_commits.append(pd.Series([repo_id,repo_row['NAME'],repo_row['OWNER'],row['REPO_ID'],row['OPEN_ISSUES']]), sort = False, ignore_index = True)
        ...
        r_commits.to_excel(r_writer, sheet_name='Sheet1', index=False)
        r_writer.save()    
